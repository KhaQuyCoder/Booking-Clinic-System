import React, { useEffect, useState } from "react";
import { useLocation, useNavigate, useParams } from "react-router-dom";
import packagesData from '../../../../data/Packagelist.json';
import { BiArrowBack } from "react-icons/bi";
import { FaTrashAlt, FaWrench } from "react-icons/fa";
import './Listdetailpackage.css';

function Listdetailpackage() {
    const location = useLocation();
    const navigate = useNavigate();
    const { id } = useParams();

    const [packages, setPackages] = useState(location.state?.packages || null);
    const [loading, setLoading] = useState(false);

    useEffect(() => {
        if (!packages) {
            setLoading(true);
            const found = packagesData.find(c => String(c.id) === String(id));
            if (found) {
                setPackages(found);
            }
            setLoading(false);
        }
    }, [packages, id]);

    const handleFix = (packages) => {
        navigate(`/quan-ly-goi/danh-sach-goi/sua-thong-tin-goi/${packages.id}`, { state: { packages } });
    }

    if (loading) return <div>Đang load...</div>;

    if (!packages) {
        return (
            <div>Không tìm thấy dữ liệu gói đăng ký. <button onClick={() => navigate(-1)}><BiArrowBack />Quay lại</button></div>
        )
    }
    return (
        <div className="listdetail">
            <div className="columnpackage">
                <div>Thông tin đăng ký</div>
                <div className="rowpackage">
                    <p><span>*Tài khoản tạo:</span> {packages.idname}</p>
                    <p><span>*Ngày tạo:</span> {packages.date || "N/A"}</p>
                </div>
                <div className="rowpackage">
                    <p><span>*ID gói đăng ký:</span> {packages.id}</p>
                    <p><span>*Tên gói đăng ký:</span> {packages.name}</p>
                </div>
                <div className="rowpackage">
                    <p><span>*Loại chu kỳ:</span> {packages.termtype}</p>
                    <p><span>*Số ngày sử dụng:</span> {packages.effectiveday}</p>
                </div>
                <div className="rowpackage">
                    <p><span>*Giá:</span> {packages.termtype}</p>
                    <p><span>*Đơn vị tiền tệ:</span> {packages.currencytype}</p>
                </div>
                <div className="rowpackage">
                    <p><span>Mô tả ngắn:</span> {packages.describe || "N/A"}</p>
                </div>
            </div>
            <div className="interacts">
                <button className="interact" onClick={() => navigate(-1)}><BiArrowBack />Quay lại</button>
                <button className="interact" onClick={() => handleFix(packages)}><FaWrench color="#2ecc71" /> Sửa</button>
                <button className="interact"><FaTrashAlt color="#D72638" /> Xóa</button>
            </div>
        </div>
    );
}
export {Listdetailpackage};